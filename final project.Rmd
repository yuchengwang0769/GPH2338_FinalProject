...
bwd.fit <- regsubsets(WALK_SCORE~., data=data[train,], method="backward", nvmax = 113)
hybrid.fit <- regsubsets(WALK_SCORE~., data=data[train,],method="seqrep", nvmax = 30)
...
knn_by_k <-  function(k) {
    ...
    fit <- knnreg(WALK_SCORE ~ ., data = traindata[fold_ind != j, ], k = k)
    pred <- predict(fit, newdata = traindata[fold_ind == j, ])
    mean((traindata$WALK_SCORE[fold_ind == j] - pred)^2)
    }))
...
knn.final <- knnreg(WALK_SCORE ~ ., data = traindata, k = best_k)
knn.pred <- predict(knn.final, newdata=testdata)
